...
switch (process.platform) {
  case 'win32':
    ...
    switch (process.arch) {
      case 'x64':
        xfuture_helper_path = path.join(xfuture_path, `package/win32-x64/sysproxy.exe`)
        break
      case 'arm64':
        xfuture_helper_path = path.join(xfuture_path, `package/win32-arm64/sysproxy.exe`)
        break
    }
    break
  case 'darwin':
...
const install = () => {
  console.log(
    ...
    `xfuture_shell_path: ${xfuture_shell_path}\n`,
    `xfuture_helper_path: ${xfuture_helper_path}\n`,
    `xfuture_resource_path: ${xfuture_resource_path}\n`,
    `log_path: ${log_path}`,
  )
  xEngine.InstallDriver(xfuture_shell_path, xfuture_helper_path, xfuture_resource_path, log_path)
...
const make_template_config_file = () => {
  try {
    ...
    console.error('xfuture', 'make template config file failed:', err?.message)
  }
}
...
const start = (url, mode, rule, rules) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      xEngine.StopTunnel()
      const isNiuniu = url?.startsWith('niuniu://')
      xEngine.SetTunModeEnable(mode === 'tun' || isNiuniu)
      xEngine.SetGlobalMode(rule === 'global' || isNiuniu)
...
const initialize = () => {
  close().catch(err => {
    console.error('xfuture', 'xfuture close on before reqady failed:', err?.message)
  })
}

const before_quit = () => {
  close().catch(err => {
    console.error('xfuture', 'xfuture close on before quit failed:', err?.message)
  })
}
