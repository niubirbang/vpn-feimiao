...
const { createConnectionChecker } = require("./util");
const {
  get_core: get_core_config,
  set_core: set_core_config,
} = require("./config");
const {
  start: x_start,
  close: x_close,
  statistics: x_statistics,
  ping: x_ping,
} = require("./service");
...
const PROXY_NODE_STATUS_CHECKING = "CHECKING";
const PROXY_NODE_STATUS_CONNECTED = "CONNECTED";
const PROXY_NODE_STATUS_FAILED = "FAILED";

let current_node_checker = null;
...
const set_subscribe = (subscribe) => {
  console.log("core", "set subscribe", JSON.stringify(subscribe));
  store.subscribe = subscribe;
  const picked = pick(subscribe, {
...
const fresh_subscribe = (subscribe) => {
  console.log("core", "fresh subscribe", JSON.stringify(subscribe));
  store.subscribe = subscribe;
...
const clear_subscribe = async () => {
  console.log("core", "clear subscribe");
  store.subscribe = [];
  store.current_names = [];
  ...
  try {
    ...
    console.error("core", "clear subscribe do close failed:", err?.message);
  }
};
const clear_current = async () => {
  console.log("core", "clear current");
  store.current_names = [];
  store.current_node = null;
  sync_store();
};
...
const run_sync_check_expired = () => {
  ...
  check_expired_interval = setInterval(() => {
    ...
    if (
      !store.current_node.usable ||
      store.current_node.expired_at <= Math.floor(Date.now() / 1000)
    ) {
      close()
        .then(() => {
          window_show({
        ...
        .catch((err) => {
          console.error(
            "core",
            "sync check expired do close failed:",
            err?.message
          );
        });
    }
...
const run_delay = () => {
  for (let _node of subscribe_to_nodes(store.subscribe)) {
    test_node_delay(_node.info);
  }
};
const retest_node_delay = (url) => {
  for (let _node of subscribe_to_nodes(store.subscribe)) {
    const node = _node.info;
    if (node.url === url) {
      store.delaies[url] = null;
      test_node_delay(node);
      break;
    }
  }
};
const test_node_delay = (node) => {
  x_ping(node.url)
    .then((delay) => {
      ...
      if (
        node?.inner?.delay_mock &&
        typeof node?.inner?.delay_mock === "function"
      ) {
      ...
      if (node.url in store.delaies) {
        ...
        } else if (delay > 0 && old_delay > 0 && old_delay > delay) {
          store.delaies[node.url] = delay;
        }
      } else {
        store.delaies[node.url] = delay;
    ...
    .catch((err) => {
      ...
      if (node.url in store.delaies) {
        ...
        store.delaies[node.url] = null;
      }
    });
  // test_delay(node.address, node.port)
  //   .then((delay) => {
  //     // console.log('core', `test node delay success: ${node.name}, ${node.address}:${node.port}, ${delay}`)
  //     if (
  //       node?.inner?.delay_mock &&
...
const start = () => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      await x_start(store.current_node.url, store.rule, store.api);
      set_config();
      do_check_node_conn();
    } catch (err) {
      reject(err);
...
const do_check_node_conn = () => {
  done_check_node_conn();

  store.current_node_status = PROXY_NODE_STATUS_CHECKING;
  sync_store();
  ...
  current_node_checker = createConnectionChecker(
  ...
  current_node_checker
    .check()
    .then(() => {
      setTimeout(() => {
        store.current_node_status = PROXY_NODE_STATUS_CONNECTED;
        sync_store();
      }, 1000);
    })
    .catch(() => {
      setTimeout(() => {
        store.current_node_status = PROXY_NODE_STATUS_FAILED;
        sync_store();
      }, 1000);
    });
};
const done_check_node_conn = () => {
  if (current_node_checker) {
    current_node_checker.stop();
  }
  store.current_node_status = null;
  current_node_checker = null;
  sync_store();
};
...
const pick_default = (subscribe = [], parent_names = []) => {
  ...
  for (let node of subscribe_to_nodes(subscribe)) {
    if (
      !node.info.usable ||
      node.info.expired_at <= Math.floor(Date.now() / 1000)
    ) {
      continue;
    }
    nodes.push(node);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (store.status == PROXY_STATUS_ON) {
      try {
        ...
        console.error("core", "get statistics failed:", err?.message);
      }
    } else {
...
const get_ipc_subscribe_item = (subscribe_item) => {
  if (!subscribe_item) {
    return subscribe_item;
  }
  let new_subscribe_item = {};
  for (let key in subscribe_item) {
    if (key === "inner") {
      continue;
    }
    if (key === "children") {
      if (subscribe_item[key] && subscribe_item[key].length > 0) {
        new_subscribe_item.children = get_ipc_node_children(
          subscribe_item[key]
        );
      } else {
        new_subscribe_item.children = [];
      }
      continue;
    }
    new_subscribe_item[key] = subscribe_item[key];
  }
  return new_subscribe_item;
};

const get_ipc_node_children = (subscribe) => {
  var new_subscribe = [];
  for (let subscribe_item of subscribe) {
    new_subscribe.push(get_ipc_subscribe_item(subscribe_item));
  }
...
const get_store = () => {
  return {
    ...
    rules: store.rules,
    smart: store.smart,
    subscribe: get_ipc_node_children(store.subscribe),
    current_choise_names: store.current_choise_names,
    current_names: store.current_names,
    current_node: get_ipc_subscribe_item(store.current_node),
    current_node_status: store.current_node_status,
...
const initialize_ipc = () => {
  ...
  registeIPCHandle("core.retest_node_delay", (e, ...v) => {
    return retest_node_delay(...v);
  });
