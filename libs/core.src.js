  ...
  check_expired_interval = setInterval(() => {
    ...
    if (!store.current_node.usable || store.current_node.expired_at <= Math.floor(Date.now() / 1000)) {
      close().then(() => {
        ...
        console.error('core', 'sync check expired do close failed:', err?.message)
      })
    }
  }, 5000)
}
...
const do_check_node_conn = () => {
  ...
  current_node_checker.check().then(() => {
    setTimeout(() => {
  ...
  for (let key in subscribe_item) {
    ...
    if (key === 'children') {
      if (subscribe_item[key] && subscribe_item[key].length > 0) {
        new_subscribe_item.children = get_ipc_node_children(subscribe_item[key])
      } else {
