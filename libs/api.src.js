...
const response_data_interceptor = (resp) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.info('api', 'response success', JSON.stringify({
...
const response_err_interceptor = (err) => {
  console.error('api', 'response failed', JSON.stringify({
...
const set_authorization_type = async (param = {
  authorization_type: null,
}) => {
  authorization_type = param.authorization_type
  clear_user()
...
const set_password = async (param = {
  password: null,
}) => {
  password = param.password
  set_authorization_file({ authorization_type, authorization, token, password })
...
const fresh_authorization_extra = () => {
  ...
  setTimeout(() => {
    obtain_authorization_extra().then(data => {
      ...
      console.error('api', 'fresh authorization extra failed:', err?.message)
    })
  }, 3000)
}
...
const fresh_subscribe = (must) => {
  obtain_subscribe().then(data => {
    ...
    console.error('api', 'fresh subscribe failed:', err?.message)
...
const clear_subscribe = async () => {
  try {
    ...
    console.error('api', 'clear subscribe failed:', err?.message)
  }
}
// ----------subscribe----------
...
const fresh_user = (must) => {
  ...
  obtain_user().then(data => {
    ...
    console.error('api', 'fresh user failed:', err?.message)
    if (must) {
      setTimeout(() => {
        fresh_user(must)
      }, 2000)
...
const initialize_version = () => {
  const do_func = () => {
    obtain_version().then(data => {
      ...
      console.error('api', 'check version failed:', err?.message)
    })
  }
...
const get_base_host_from_remote = () => {
  return new Promise((resolve, reject) => {
    axios.get('https://shxiongying.oss-cn-shanghai.aliyuncs.com/a.log').then(data => {
...
const initialize = async () => {
  ...
  console.info('api', 'host in config:', host)

  try {
    host = await get_base_host_from_remote()
    console.info('api', 'host in remote:', host)
  } catch (err) {
    console.error('api', `get host from remote failed:`, err?.message)
...
const initialize_ipc = () => {
  ...
  registeIPCHandle('api.login_by_device', (e, ...v) => {
    return login_by_device(...v)
  })
    ...
    try {
      ...
      switch (method) {
        ...
        case 'email':
          ...
          break
        default:
          reject('api_unsupport_sms_method')
          return
      }
      await api.request({
        method: api_method,
...
const registe = (method, param) => {
  return new Promise(async (resolve, reject) => {
    try {
      let api_method = null
      let api_url = null
      let api_param = null
      let api_authorization_type = null
      let password = null
      switch (method) {
        case 'phone':
          api_method = 'POST'
          ...
          api_param = {
            tel: param?.phone,
            code: param?.phoneCode,
            password: param?.password,
          }
          api_authorization_type = AUTHORIZATION_TYPE_USER
          password = param?.password
          break
        case 'email':
          api_method = 'POST'
          ...
          break
        default:
          reject('api_unsupport_registe_method')
          return
      }
      ...
      await set_authorization({
        authorization_type: api_authorization_type,
    ...
    try {
      ...
      switch (method) {
        ...
        case 'email_sms':
          ...
          break
        default:
          reject('api_unsupport_login_method')
          return
      }
...
const login_by_device = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await set_authorization({
        authorization_type: AUTHORIZATION_TYPE_DEVICE,
        authorization: data.auth_data,
        token: data.token,
        password: null,
      })
...
const forget_password = (method, param) => {
  return new Promise(async (resolve, reject) => {
    try {
      let api_method = null
      let api_url = null
      ...
      switch (method) {
        ...
        case 'email':
          ...
          break
        default:
          reject('api_unsupport_forget_method')
          return
      }
      await api.request({
        method: api_method,
    ...
    try {
      ...
      switch (method) {
        ...
        case 'email':
          ...
          break
        default:
          reject('api_unsupport_bind_method')
          return
      }
      await api.request({
        method: api_method,
...
const receive_free = () => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.error('api', 'receive free do fresh user failed:', err?.message)
    }
    try {
      await refresh_subscribe()
    } catch (err) {
      console.error('api', 'receive free do refresh subscribe failed:', err?.message)
    }
    resolve()
  })
}
...
const subscribe_delay_mocks = {
  svip: (delay) => {
    return parseFloat(delay) / 2
  },
  vip: null,
  free: null,
}
  ...
  for (let item of data) {
    ...
    for (let url of item.node) {
      ...
      children.push({
        ...
        extra: {},
        inner: {
          delay_mock: subscribe_delay_mocks[item.type] || null,
        },
      })
    }
    subscribe.push({
