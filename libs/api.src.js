...
const set_authorization_type = async (
  param = {
    authorization_type: null,
  }
) => {
...
const set_version = (
  ...
  version.is_latest =
    compareVersion(version.current_version, version.server_version) >= 0;
...
const get_base_host_from_remote = () => {
  return new Promise((resolve, reject) => {
    axios
      .get("https://shxiongying.oss-cn-shanghai.aliyuncs.com/a.log")
      .then((data) => {
...
const initialize = async () => {
  ...
  api.interceptors.response.use(
    response_data_interceptor,
    response_err_interceptor
  );
...
const receive_free = () => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.error(
        "api",
        "receive free do refresh subscribe failed:",
        err?.message
      );
    }
    resolve();
