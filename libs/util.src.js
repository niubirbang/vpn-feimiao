...
const proxyAgent = new SocksProxyAgent("socks5://127.0.0.1:40008");
...
function createConnectionChecker(url, timeout, interval, useScoksProxy) {
  let isStopped = false;
  ...
  function check() {
    function attemptRequest() {
      if (isStopped) {
        return Promise.reject(new Error("stoped"));
      }
      let option = {
        timeout: 5000,
      };
      if (useScoksProxy) {
        option.httpsAgent = proxyAgent;
      }
      return axios
        .get(url, option)
        .then((response) => {
