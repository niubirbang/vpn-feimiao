...
import { VIPBG, SVIPBG, VIPPermission1, VIPPermission2, VIPPermission3, VIPPermission4, SVIPPermission1, SVIPPermission2, SVIPPermission3, SVIPPermission4 } from '@/util/assets'
...
const pay = () => {
  ...
  window.box_api.payOrderByPlan(
    currentPrice.value.plan_id, currentPrice.value.period, currentPayment.value.id,
  ).then((data) => {
    pay_info.value = data
    loadPayQRDialog()
  }).catch(err => {
...
<template>
  ...
  >
    ...
    <div class="payments">
      ...
      >
        ...
        <Iconfont
          ...
          :name="payment.id == currentPayment.id ? 'radio-selected' : 'radio-unselect'"
        />
      </div>
    </div>
    <div
    ...
    >
      ...
      <p>游客状态只能一台设备登录并且充值出现丢失情况无法找回， 如需其他设备登录或订单找回，请绑定账号。</p>
    </div>
    <div>
      <el-button
        class="bind-pay"
