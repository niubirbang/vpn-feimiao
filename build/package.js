...
// win32 x64
const win32_x64_install = path.join(dist_path, `MH-SPEED-win-x64-${version}-setup.exe`)
const win32_x64_asar = path.join(dist_path, `win-unpacked/resources/app.asar`)

// win32 arm64
const win32_arm64_install = path.join(dist_path, `MH-SPEED-win-arm64-${version}-setup.exe`)
const win32_arm64_asar = path.join(dist_path, `win-arm64-unpacked/resources/app.asar`)

// darwin x64
const darwin_x64_install = path.join(dist_path, `MH-SPEED-mac-x64-${version}-setup.dmg`)
const darwin_x64_asar = path.join(dist_path, `mac/${productName}.app/Contents/Resources/app.asar`)

// darwin arm64
const darwin_arm64_install = path.join(dist_path, `MH-SPEED-mac-arm64-${version}-setup.dmg`)
