...
const installers = {
  "win32-x64": path.join(dist_path, `MH-SPEED-win-x64-${version}-setup.exe`),
  "win32-arm64": path.join(
    dist_path,
    `MH-SPEED-win-arm64-${version}-setup.exe`
  ),
  "darwin-x64": path.join(dist_path, `MH-SPEED-mac-x64-${version}-setup.dmg`),
  "darwin-arm64": path.join(
    dist_path,
    `MH-SPEED-mac-arm64-${version}-setup.dmg`
  ),
};
const asars = {
